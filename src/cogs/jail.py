...
class Jail(commands.Cog):
    ...
    async def jail_check(self):
        ...
        for guild in self.bot.guilds:
            guild_id = guild.id
            self.logger.debug(
                guild_id, f"Jail Check for guild {guild.name}.", cog=self.__cog_name__
            )
            ...
            for jail in active_jails:
                ...
                self.logger.debug(
                    guild_id,
                    f"Jail Check for {member.name}. Duration: {duration}, Remaining: {remaining}",
                ...
                self.logger.log(
                    guild_id,
                    f"User {member.name} was released from jail after {BotUtil.strfdelta(duration, inputtype='minutes')}.",
                    cog=self.__cog_name__,
                )
                ...
                for channel_id in jail_channels:
                    ...
                    await channel.send(
                        f"<@{member.id}> was released from jail after {BotUtil.strfdelta(duration, inputtype='minutes')}."
                    )
    ...
    async def on_ready(self):
        ...
        if len(jails) > 0:
            self.logger.log(
                "init",
                f"Found {len(jails)} ongoing jail sentences.",
                cog=self.__cog_name__,
            )
        ...
        for jail in jails:
            ...
            if member is None:
                self.logger.log(
                    "init",
                    f"Member or guild not found, user {member_id} was marked as released.",
                    cog=self.__cog_name__,
                )
            ...
            if member.get_role(jail_role) is None:
                self.logger.log(
                    "init",
                    f"Member {member.name} did not have the jail role, applying jail role now.",
                    cog=self.__cog_name__,
                )
            ...
            self.logger.log(
                "init",
                f"Continuing jail sentence of {member.name} in {guild.name}. Remaining duration: {BotUtil.strfdelta(remaining, inputtype='minutes')}",
                cog=self.__cog_name__,
            )
    ...
    async def jail(
        ...
        success = await self.jail_manager.jail_user(
            guild_id, interaction.user.id, user, duration
        )
        ...
        if not success:
            await self.bot.command_response(
                self.__cog_name__,
                interaction,
                f"User {user.name} is already in jail.",
        ...
        await self.jail_manager.announce(
            interaction.guild,
            f"<@{user.id}> was sentenced to Jail by <@{interaction.user.id}> . They will be released <t:{release}:R>.",
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"User {user.name} jailed successfully.",
            args=[user.name, duration],
        )

    @app_commands.command(name="release", description="Resease a user from jail.")
    @app_commands.describe(user="Resease this user from jail.")
    @app_commands.guild_only()
    async def release(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if user.get_role(jail_role) is None:
            await self.bot.command_response(
                self.__cog_name__,
                interaction,
                f"User {user.display_name} is currently not in jail.",
                args=[user],
            )
            return
        ...
        if (
            ...
            await self.bot.command_response(
                self.__cog_name__, interaction, "lmao", args=[user]
            )
            return
        ...
        if not response:
            await self.bot.command_response(
                self.__cog_name__,
                interaction,
                f"Something went wrong, user {user.display_name} could not be released.",
                args=[user],
            )
            return

        response = (
            f"<@{user.id}> was released from Jail by <@{interaction.user.id}>. "
            + response
        )
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"User {user.display_name} released successfully.",
            args=[user],
        )

    group = app_commands.Group(
        name="degenjail", description="Subcommands for the Jail module."
    )

    @group.command(
        name="settings",
        description="Overview of all jail related settings and their current value.",
    )
    @app_commands.check(__has_permission)
    async def get_settings(self, interaction: discord.Interaction):
        output = await self.settings_manager.get_settings_string(
            interaction.guild_id, SettingsManager.JAIL_SUBSETTINGS_KEY
        )
        await self.bot.command_response(self.__cog_name__, interaction, output)

    @group.command(
        name="toggle", description="Enable or disable the entire jail module."
    )
    ...
    async def set_toggle(
        ...
        await self.settings_manager.set_jail_enabled(
            interaction.guild_id, enabled == "on"
        )
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Jail module was turned {enabled}.",
            args=[enabled],
        )

    @group.command(
        name="add_channel", description="Enable jail interactions for a channel."
    )
    ...
    async def add_channel(
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Added {channel.name} to jail channels.",
            args=[channel.name],
        )

    @group.command(
        name="remove_channel", description="Disable jail interactions for a channel."
    )
    ...
    async def remove_channel(
        ...
        await self.settings_manager.remove_jail_channel(
            interaction.guild_id, channel.id
        )
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Removed {channel.name} from jail channels.",
            args=[channel.name],
        )
    ...
    async def add_mod_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Added {role.name} to jail moderators.",
            args=[role.name],
        )

    @group.command(
        name="remove_mod_role", description="Remove jail privileges from a role."
    )
    ...
    async def remove_mod_role(
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Removed {role.name} from jail moderators.",
            args=[role.name],
        )

    @group.command(
        name="set_jailed_role", description="Sets the role for jailed people."
    )
    @app_commands.describe(role="The role for jailed users.")
    @app_commands.check(__has_permission)
    async def set_jailed_role(
        self, interaction: discord.Interaction, role: discord.Role
    ):
        ...
        await self.bot.command_response(
            self.__cog_name__,
            interaction,
            f"Jail role was set to `{role.name}` .",
            args=[role.name],
        )

    @group.command(
        name="setup", description="Opens a dialog to edit various jail settings."
    )
    @app_commands.check(__has_permission)
    async def setup(self, interaction: discord.Interaction):
        ...
        modal = SettingsModal(
            ...
            "Settings for Jail Features",
        )

        await modal.add_field(
            guild_id,
            SettingsManager.JAIL_SUBSETTINGS_KEY,
            SettingsManager.JAIL_SLAP_TIME_KEY,
            int,
        )
        await modal.add_field(
            guild_id,
            SettingsManager.JAIL_SUBSETTINGS_KEY,
            SettingsManager.JAIL_PET_TIME_KEY,
            int,
        )
        await modal.add_field(
            guild_id,
            SettingsManager.JAIL_SUBSETTINGS_KEY,
            SettingsManager.JAIL_FART_TIME_MIN_KEY,
            int,
            allow_negative=True,
        )
        await modal.add_field(
            guild_id,
            SettingsManager.JAIL_SUBSETTINGS_KEY,
            SettingsManager.JAIL_FART_TIME_MAX_KEY,
            int,
        )
        await modal.add_field(
            guild_id,
            SettingsManager.JAIL_SUBSETTINGS_KEY,
            SettingsManager.JAIL_BASE_CRIT_RATE_KEY,
            float,
        )
        ...
        modal.add_constraint(
            ...
            lambda a, b: a <= b,
            "fart minimum must be smaller than fart maximum.",
        )
        modal.add_constraint(
            [SettingsManager.JAIL_BASE_CRIT_RATE_KEY],
            lambda a: a >= 0 and a <= 1,
            "crit rate must be between 0 and 1.",
        )
