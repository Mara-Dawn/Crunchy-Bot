...
class JailManager(Service):
                ...
                match inventory_event.item_type:
                    case ItemType.EXPLOSIVE_FART:
                        if inventory_event.amount <= 0:
                            ...
                            await self.random_jailing(
                                guild, event.get_causing_user_id()
                            )
    ...
    async def announce(
        self, guild: discord.Guild, message: str, *args, **kwargs
    ) -> str:
    ...
    async def jail_user(
        self, guild_id: int, jailed_by_id: int, user: discord.Member, duration: int
    ) -> bool:
        ...
        time_now = datetime.datetime.now()
        event = JailEvent(
            time_now, guild_id, JailEventType.JAIL, jailed_by_id, duration, jail.id
        )
    ...
    async def release_user(
        ...
        if len(affected_jails) > 0:
            ...
            time_now = datetime.datetime.now()
            event = JailEvent(
                time_now, guild_id, JailEventType.RELEASE, released_by_id, 0, jail.id
            )
            await self.controller.dispatch_event(event)
    ...
    async def random_jailing(self, guild: discord.Guild, member_id: int):
        ...
        for victim in victims:
            ...
            success = await self.jail_user(guild_id, member_id, member, duration)
