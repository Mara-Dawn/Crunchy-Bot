...
class StatisticsEmbed(discord.Embed):
    ...
    def __init__(self, bot: CrunchyBot, interaction: discord.Interaction, user: discord.Member, user_statistics: UserStats):
        ...
        self.add_field(name="__Interaction Count__", value="This is how much you've been spamming the server.", inline=False)
        
        self.add_field(name="Slaps:", value=f"**{user_statistics.get_slaps_recieved()}** recieved\n**{user_statistics.get_slaps_given()}** given", inline=True)
        self.add_field(name="Pets:", value=f"**{user_statistics.get_pets_recieved()}** recieved\n**{user_statistics.get_pets_given()}** given", inline=True)
        self.add_field(name="Farts:", value=f"**{user_statistics.get_farts_recieved()}** recieved\n**{user_statistics.get_farts_given()}** given", inline=True)
    
        self.add_field(name="__Top Interactions__", value="These are the people who interacted with you the most.", inline=False)
        ...
        top_slappers = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_slappers]
        top_petters = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_petters]
        top_farters = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_farters]
        ...
        top_slapperd = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_slapperd]
        top_petterd = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_petterd]
        top_farterd = [f'**{amount}** {BotUtil.get_name(bot, interaction.guild_id, user_id)}' for (user_id, amount) in top_farterd]
        ...
        self.add_field(name="Times in timeout:", value=f'**{user_statistics.get_timeout_count()}**', inline=True)
        self.add_field(name="Total timeout:", value=BotUtil.strfdelta(user_statistics.get_timeout_total(), inputtype='seconds'), inline=True)
        
        self.add_field(name="__Bonus Stats__", value="Your other achievements.", inline=False)
